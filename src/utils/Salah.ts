...
class Salah {
  ...
  private sunCoords(jd: number): [number, number] {
    ...
    let L = DMath.normalizeAngle(q + (1.915 * DMath.sin(g)) + (0.020 * DMath.sin(2.0 * g)));
   ...
   */
  private horizonHourAngle(angle: number, jd: number, direction: 1 | -1) {
    let [_, decl] = this.sunCoords(jd);
  
    let Ta = (1 / 15) * DMath.acos(
  ...
  private shadowLengthHour(length: number, jd: number) {
    let [_, decl] = this.sunCoords(jd);
    let At = (1 / 15) * DMath.acos(
      (DMath.sin(DMath.acot(length + DMath.tan(this.lat - decl))) - (DMath.sin(this.lat) * DMath.sin(decl)))
          / (DMath.cos(this.lat) * DMath.cos(decl)),
    );
  ...
  private midnight(date: Date): Date {
    ...
    let mid = Duration.fromDifference(sunsetToday, sunriseTomorrow).getMilliseconds() / 2;
